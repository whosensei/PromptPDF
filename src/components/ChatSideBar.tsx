...
const ChatSideBar = ({ chats, chatId }: Props) => {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-orange/5 p-4 text-charcoal">
      ...
      <div className="mt-8 mb-3">
        <h3 className="text-sm font-medium text-charcoal/80 uppercase tracking-wider">YOUR DOCUMENTS</h3>
      </div>
          ...
          <Link key={chat.id} href={`/chat/${chat.id}`}>
            <div
              className={cn("rounded-md p-3 mt-2 flex items-center transition-all duration-200", {
                ...
                "bg-white text-charcoal hover:bg-orange/10": chat.id !== chatId,
              })}
            >
      ...
      <div className="mt-auto pt-4 border-t border-orange/20">
        <div className="text-xs text-charcoal/60 mb-2">© {new Date().getFullYear()} PromptPDF</div>
      </div>
    </div>
  )
}
